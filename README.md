...
```new``` 명령은 프로젝트나 파일을 생성한다. 생성할 수 있는 프로젝트/파일의 템플릿은 탬플릿 지정 없이 ```dotnet new``` 명령만으로 실행하면 생성 가능한 템플릿의 종류를 확인할 수 있다.

여기서는 ```sln``` 템플릿을 생성한다. 다들 알겠지만, 비주얼 스튜디오 솔루션 파일 템플릿이다.
...
여기서는 위에서 만든 웹 프로젝트와 라이브러리 프로젝트를 솔루션 파일에 추가한다.
...
```add``` 명령은 프로젝트에 다른 프로젝트로의 참조나 NuGet 패키지의 참조를 추가할 때 사용한다.

여기서는 웹 프로젝트에 라이브러리 프로젝트를 참조하도록 설정했다.

참고로 ```add``` 명령의 ```reference``` 명령이 아닌 ```package``` 명령으로 nuget 패키지를 참조하도록 설정할 수 있다. 설치할 패키지는 [NuGet](https://www.nuget.org) 사이트에서 검색하면 된다. NuGet 사이트에서 참조 설정 명령어까지 표시해준다. 굳이 예를 들자면 웹 프로젝트에 JSON 처리를 위한 Newtonsoft.Json 패키지를 참조하려면 아래와 같이 하면 된다.

(굳이 얘기하자면 Maven Repository 사용하는 기분으로 쓰면 된다는 소리다)
...
다만, 문서에 보면 ```dotnet-sdk-x.y``` 패키지를 설치하도록 되어있는데, 개발이 아니라 서비스 구동만을 위해서라면 굳이 SDK 패키지를 설치할 필요는 없다. ```dotnet-runtime-x.y``` 패키지를 설치하는 것으로 충분한다.
...
닷넷은 배포 패키지 안에 실행환경이 포함된 ```자체포함 어플리케이션(Self-contained Application)```을 만들 수 있다. 자체포함 어플리케이션으로 만들면, 배포할 서버에 닷넷 실행환경을 굳이 설치하지 않아도 된다는 장점이 있다. 이 앱의 실행환경이 다른 서비스와 격리된다는 점은 덤. 자바 9부터 추가된 [jlink](https://github.com/MasakiKun/jlink-example)와 비슷한 느낌이다.

개인적으로는 간편하게 배포하고 실행할 수 있는 self-contained 어플리케이션이 배포와 운영에 더 유리하지 않을까 생각해본다. 뭐, 필드에서 어떻게 받아들여질지는 모르겠지만.

self-contained 어플리케이션을 만들기 위해서는 publish 명령에 ```--runtime``` 옵션으로 배포할 서버의 운영환경을 지정해주면 된다. 닷넷코어에서 지원하는 런타임셋은 [.NET Core RID 카탈로그](https://docs.microsoft.com/ko-kr/dotnet/core/rid-catalog) 페이지를 참고한다.

서론이 길었는데, 요는 publish 명령에 ```--runtime``` 옵션을 줘서 배포셋을 만들면, 실행할 환경에서는 닷넷 실행환경을 설치할 필요가 없다는 것이다.

    dotnet publish --configuration Release --output publish --runtime linux-x64

위 명령어는 배포 패키지에 64비트 리눅스 실행환경을 포함해서 배포셋을 만든다.

서버에서는 배포셋을 업로드한 후, 진입점 프로젝트의 이름을 가진 파일에 실행권한을 준 뒤, 그 파일을 그대로 실행하면 된다.
...
[VSCode](https://code.visualstudio.com/)는 각종 플러그인을 설치해서 쓸 수 있는 범용 개발툴이라는 느낌이 강하지만, 마이크로소프트에서 만든거라 그런지 특히 C# 개발할때 굉장히 편리하다. RAD 기능만 제거된 간이 비주얼 스튜디오를 쓰는 수준.

VSCode에서 닷넷코어 프로젝트를 개발하기 위해서는, 프로젝트 폴더를 연 뒤 솔루션 파일을 연다. C# 플러그인이 설치되어 있지 않다면 ```The 'C#' extension is recommemded for this file type.```이라는 메시지가 표시되므로 Install을 클릭해서 C# 플러그인을 설치한다. 잠시 기다리면 ```Required assets to build and debug are missing from '프로젝트'. Add them?```이라고 표시되므로 Yes를 클릭하면 자동으로 실행환경을 잡아준다.
...
Gradle을 쓸때는 빌드 스크립트가 잘못되면 프로젝트 만들때부터 막혀버리는 경우도 있다는걸 생각해보면 일장일단인거 같다. 다만 대형 프로젝트에서 이렇게는 도저히 못하겠다. 하루빨리 빌드 스크립트가 도입되던지 해얄듯. (마소도 바보는 아니니, 이미 있는데 내가 모르는걸지도)

실행환경 포함 셋은 jlink랑 비슷하지만, 운영체제별로 세세하게 나뉘어져 있다는게 신기했다. 예를 들어서 같은 리눅스라도 ```linux-x64```뿐 아니라 우분투용 ```ubuntu.18.04-x64```, 레드햇용 ```rhel.7.4-x64``` 등등등. 운영체제별로 얼마나 더 최적화해주는지는 잘 모르겠지만, 선택지가 늘어난다는건 좋은거 같다. 솔직히 자바에서 jlink 쓰려면 아직 미성숙 기술인 JPMS가 강제되는 바람에 써먹기가 힘들다는걸 생각해보면, 이 부분에서는 닷넷이 자바보다도 더 나은듯.

하위 호환성은 조금 아쉽다. 아니, 아무리 그래도 그렇지 2.1.5에서 만든게 2.1.8에서 안돌아간다고 하면 이건 너무하는거 같은데😓

내가 하위버전에서 돌린것도 아닌데 이래버리면, 쩝. 데비안 계열을 쓰게 되면 ```apt```로 자주자주 패키지를 업데이트 할수도 있다는걸 생각해보면, 어느날 갑자기 엿먹고 서비스 구동 불능 사태가 오는 경우도 왕왕 있을듯😑 결론은 self-contained 배포셋 만만세다.
