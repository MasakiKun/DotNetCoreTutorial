...
실행환경 포함 셋은 jlink랑 비슷하지만, 운영체제별로 세세하게 나뉘어져 있다는게 신기했다. 예를 들어서 같은 리눅스라도 ```linux-x64```뿐 아니라 우분투용 ```ubuntu.18.04-x64```, 레드햇용 ```rhel.7.4-x64``` 등등등. 운영체제별로 얼마나 더 최적화해주는지는 잘 모르겠지만, 선택지가 늘어난다는건 좋은거 같다. 솔직히 자바도 jlink 쓰려면 아직 미성숙 기술인 JPMS가 강제되는 바람에 아직 써먹기가 힘들다는걸 생각해보면, 이 부분에서는 닷넷이 자바보다도 더 나은듯.
